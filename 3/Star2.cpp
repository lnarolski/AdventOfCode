...
struct intersection
{
    long posX;
    long posY;
    long stepsToIntersectionFirstWire;
    long stepsToIntersectionSecondWire;
    long sumOfSteps;
};
...
void addStepsToVector(long posX, long posY, vector <intersection*> intersections, int numOfWire, long numOfSteps)
{
    for (int i = 0; i < intersections.size(); ++i)
    {
        if (intersections[i]->posX == posX && intersections[i]->posY == posY)
        {
            if (numOfWire == 0)
                intersections[i]->stepsToIntersectionFirstWire = numOfSteps;
            else
                intersections[i]->stepsToIntersectionSecondWire = numOfSteps;
            return;
        }
    }
}

long findMinimumNumberOfSteps(vector <intersection*> intersections)
{
    long minimumNumberOfSteps = LONG_MAX;
    for (int i = 0; i < intersections.size(); ++i)
    {
        intersections[i]->sumOfSteps = intersections[i]->stepsToIntersectionFirstWire + intersections[i]->stepsToIntersectionSecondWire;
        if (intersections[i]->sumOfSteps < minimumNumberOfSteps)
            minimumNumberOfSteps = intersections[i]->sumOfSteps;
    }
...
int main(int argc, char *argv[])
{
    ...
    // Calculating number of steps to every intersection for wire 1 and wire 2
    lineNumber = 0;
    input.open("input.txt", fstream::in);
    while (getline(input, fileLine))
    {
            ...
            else
            {
                ...
                case UP:
                    for (int k = 0; k < stoi(buffer); ++k)
                    {
                        ...
                        {
                            ++stepsToIntersection;
                            if (grid[posX][posY] == INTERSECTION)
                            {
                                addStepsToVector(posX, posY, intersections, 0, stepsToIntersection);
                            }
                        }
                        else if (lineNumber == 1)
                        {
                            ...
                            if (grid[posX][posY] == INTERSECTION)
                            {
                                addStepsToVector(posX, posY, intersections, 1, stepsToIntersection);
                            }
                        }
                    }
                    break;
    ...
    cout << "Number of steps: " << findMinimumNumberOfSteps(intersections) << endl;
