...
int main(int argc, char* argv[])
{
	...
	const int widthArea = 1000;
	const int heightArea = 1000;
	...
	while (!computerStop)
	{
		//std::cout << "Robot position: " << robotPosition.x << " " << robotPosition.y << std::endl;
	...
	for (long y = 0; y < heightArea; ++y)
	{
		...
		{
			if (paintedOnce[y][x] == true)
				++countOfPaintedOnce;
		}
	}

	std::cout << std::endl << "countOfPaintedOnce: " << countOfPaintedOnce << std::endl;
}